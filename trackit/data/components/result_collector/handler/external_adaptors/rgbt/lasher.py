...
# Add support for LasHeR evaluation
...
from .utils import serial_process, iou, cle, normalize_cle, SequenceMetric, DatasetMetrics

from ....progress_tracer import EvaluationProgress
from ... import EvaluationResultHandler


class LasHeREvaluationMetricCalculator:
    def __init__(self, npr_thr=np.linspace(0, 0.5, 51), npr_val_idx=20,
                 pr_thr=np.linspace(0, 50, 51), pr_val_idx=20, sr_thr=np.linspace(0, 1, 21)):
        self._dataset = LasHeR()
        self._npr_thr = npr_thr
        self._npr_val_idx = npr_val_idx
        self._pr_thr = pr_thr
    ...
    def __call__(self, seq_name: str, predicted_bboxes: np.ndarray) -> SequenceMetric:
        try:
            gt = self._dataset[seq_name]
        except:
            gt = self._dataset[seq_name]['visible']

        processed_bboxes = [r for r in predicted_bboxes]
        processed_bboxes[0] = gt[0]
        processed_bboxes = processed_bboxes[:len(gt)]

        for i in range(1, len(gt)):
            if processed_bboxes[i][2] <= 0 or processed_bboxes[i][3] <= 0:
                processed_bboxes[i] = processed_bboxes[i - 1].copy()

        npr_res = serial_process(normalize_cle, processed_bboxes, gt)
        ...
        npr_cell = []
        for i in self._npr_thr:
            npr_cell.append(np.sum(npr_res <= i) / len(npr_res))
        npr_val = npr_cell[self._npr_val_idx]
        ...
        metric = SequenceMetric(
            ...
            normalized_precision_score=npr_val,
...
class LasHeREvaluationToolFileWriter:
...
class LasHeREvaluationToolAdaptor(EvaluationResultHandler):
    def __init__(self, tracker_name: str, output_folder: str, file_name: str = 'LasHeR', rasterize_bbox: bool = True):
        self.output_folder = output_folder

        if output_folder is not None:
            self._writer = LasHeREvaluationToolFileWriter(output_folder, file_name)

        self._tracker_name = tracker_name
        self._rasterize_bbox = rasterize_bbox
        self._metric_calculator = LasHeREvaluationMetricCalculator()
        self._dataset_metrics = DatasetMetrics(compute_npr=True)
    ...
    def accept(self, evaluation_results: Sequence[SequenceEvaluationResult_SOT],
        ...
        for evaluation_result, evaluation_progress in zip(evaluation_results, evaluation_progresses):
            ...
            print(
                f'{evaluation_result.sequence_info.sequence_name}: success {metric.success_score: .04f}, prec {metric.precision_score: .04f}, norm_prec {metric.normalized_precision_score: .04f}')
